...
function Register(){
  ...
  const disabledStyle = validateUser(user).hasErrors ? 'bg-blue-300 hover:bg-blue-300' : 
  'hover:bg-opacity-90 bg-blue-500'
        ...
        sm:min-w-[500px] flex-1 w-full'>
              ...
              <div className='flex flex-col sm:flex-row gap-1 flex-1 w-full sm:gap-6'>
                <InputComponent inputProp={{ name: user.firstname, type: "text", label: "Provide Firstname", 
                    field: "firstname", placeholder: "Jhon", handleInputChange }}/>
                <InputComponent inputProp={{ name: user.lastname, type: "text", label: "Provide Lastname", 
                    field: "lastname", placeholder: "Perera", handleInputChange }}/>
              </div>
              <InputComponent inputProp={{ name: user.username, type: "email", label: "Provide Email", 
                field: "username", placeholder: "JhonPerera123@gmail.com", handleInputChange }}/>
              <InputComponent inputProp={{ name: user.tel, type: "number", label: "Provide Tel", 
                field: "tel", placeholder: "+94259874135", handleInputChange }}/>
              ...
              <InputComponent inputProp={{ name: user.dob, type: "date", label: "Provide Date of birth", 
                field: "dob", handleInputChange }}/>
              <div className='flex flex-col sm:flex-row gap-1 flex-1 w-full sm:gap-6'>
                <InputComponent inputProp={{ name: user.password, type: "password", label: "Password", 
                    field: "password", placeholder: "Enter Your Password", handleInputChange }}/>
                <InputComponent inputProp={{ name: user.confirmPassword, type: "password", label: "Confirm Password", 
                    field: "confirmPassword", placeholder: "Confirm Your Password", handleInputChange }}/>
              </div>
              <button disabled={validateUser(user).hasErrors} onClick={signUp} type="button" className={`${disabledStyle} 
